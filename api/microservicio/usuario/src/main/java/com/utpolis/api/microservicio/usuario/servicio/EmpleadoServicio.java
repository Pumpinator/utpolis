...
public class EmpleadoServicio {
    ...
    public Iterable<UsuarioDto> listar() {
        return ((List<Usuario>) usuarioRepositorio.findAllByRolNombre(Roles.EMPLEADO.name())).stream().map(this::construirDto).collect(Collectors.toList());
    }

    public Page<UsuarioDto> paginar(Pageable pageable) {
        return usuarioRepositorio.findAllByRolNombre(Roles.EMPLEADO.name(), pageable).map(this::construirDto);
    }
    ...
    public UsuarioDto crear(UsuarioDto usuario) {
        ...
        return construirDto( // Construir DTO de usuario
                usuarioRepositorio.save(Usuario.builder() // Con el usuario guardado en el repositorio
                        ...
                        .rol(rolRepositorio.findByNombre(Roles.EMPLEADO.name()).orElseThrow(() -> new RuntimeException(String.format("Rol \'%s\' no encontrado", Roles.EMPLEADO.name())))
                        )
    ...
    public UsuarioDto eliminar(Long id) {
        ...
        if (!usuario.getRol().getNombre().equals(Roles.EMPLEADO.name()))
    ...
    public void validar(UsuarioDto usuario) {
        ...
        if (usuario.getRol() != null && !(usuario.getRol().equals(Roles.ADMINISTRADOR.name()) || usuario.getRol().equals(Roles.EMPLEADO.name()) || usuario.getRol().equals(Roles.EMPLEADO.name())))
